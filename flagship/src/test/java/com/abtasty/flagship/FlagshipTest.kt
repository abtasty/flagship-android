...
class FlagshipInstrumentedTest2 {
    ...
    fun writeInstanceProperty(instance: Any, propertyName: String, value: Any) {
        val property =
            instance::class.members.first { it.name == propertyName } as KMutableProperty<*>
    ...
    fun test_C_updateContext() {
        ...
        val campaignRule = HttpInterceptor.Rule(CAMPAIGNS_URL) { request, calls ->
            ...
            try {
                ...
                Assert.assertTrue(
                    requestBody.getJSONObject("context")
    ...
    fun test_K_bucketing_start() {
        ...
        try {
            Flagship.builder(appContext, sdk_env_id, sdk_api_key)
                ...
                .withVisitorContext(
                    hashMapOf(
                        "sdk_deviceModel2" to "Google Pixel 4924"
                    )
                )
                .start()
        } catch (e: Exception) {
        ...
        interceptor.removeAllRules()
        lock2.await()
        Assert.assertTrue(Flagship.getModification("target", "default", false) == "is not")
    }
